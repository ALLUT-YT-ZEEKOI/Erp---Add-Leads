...
(function ($) {
    ...
    $.widget("jHtree.jHTree", {
        ...
        _treeDarg: function () {
            ...
            $("li", ".tree-container .tree").draggable({
        ...
        _treeDrop: function () {
            var thisparent = this;
            $("li", ".tree-container .tree").droppable({

                greedy: true,
                accept: ".tree-container .tree ul > li",
        ...
        _interactionEvents: function () {
            ...
            $(".tree-container .tree").on('mouseleave', '.tnode', function () {
            ...
            $(".tree-container .tree").on("click", ".trcont > .ui-widget-header", function () {
                $(this).parent().parent().find('.trchl').slideToggle('slow', "easeOutBounce", function () {
                    // Animation complete.
                });
            });

            $(".tree-container .tree").on("click", "div[data-func]", function () {
                var objbtn = $(this);
                var objfocus = objbtn.parent().closest('li');
                var objother = objfocus.parent().find('> li');
                var targetobjs = $(objother).not(objfocus);
        ...
        _zoomer: function (treeDiv) {
            ...
            $("#zmrslidr").slider({
                ...
                slide: function (event, ui) {
                    $("#zmrval").val(ui.value);
                    if (brwstp == true) {
                        $('.tree-container .tree').css('MozTransform', 'scale(' + ui.value + ')');
                    } else {

                        $('.tree-container .tree').css('zoom', ' ' + ui.value + '%');
                    }
                }
            });
