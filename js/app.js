$(document).ready(function() {
    var typingTimer;
    $(document).on('keyup', '.contact_modal  [name="mobile"]', function () {

        clearTimeout(typingTimer);
        typingTimer = setTimeout(function () {
            console.log('phone-number-typed-end');
        }, 2000);
    });

    $(document).on('click','.get-user-suggestion', function () {
        $phone = $(this).closest('form').find('[name="mobile"]').val();
        $.ajax({
            url: $(this).data('href2'),
            dataType: 'json',
            context : $(this),
            ...
            success: function(result) {
                if(result.suggestions.length > 0){
                    ...
                    $(this).closest('form').find('[name="user_id[]"]').val($vals).change();
                    $(this).closest('form').find('[name="user_id[]"]').select2("val", $vals);

                    $this = $(this);
                    $this.closest('form').find('.user_select_ajax').html('').trigger('change');
                    $.each(result.data, function(k,v){
                        var newOption = new Option(v, k, false, true);
                        $this.closest('form').find('.user_select_ajax').append(newOption).trigger('change');
                    });
                }
                else{
                    toastr.success("No suggestions for this Lead");
                }
            },
        });
    });

    $('.customer_id_ajax').select2({
        ajax: {
    ...
    $(document).on('click','.copyfrombilling_btn' , function () {
        $address = '';
        $address = $address+ $('[name="first_name"]').val() +' '+$('[name="middle_name"]').val() +' '+$('[name="last_name"]').val() +'\n';
        $address = $address+ $('[name="address_line_1"]').val() +'\n';
        $address = $address+ $('[name="address_line_2"]').val() +'\n';
        $address = $address+ $('[name="city"]').val() +'\n';
        $address = $address+ $('[name="state"]').val() +'\n';
        $address = $address+ $('[name="country"]').val() +'\n';
        $address = $address+ 'Pin : '+ $('[name="zip_code"]').val() +'\n';
        $address = $address+ 'Mobile: '+$('[name="mobile"]').val();
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == true) {
                    ...
                    var data = {
                        id : result.data.id,
                        text: result.show_name || result.data.actual_name
                    };
                    var newOption = new Option(data.text, data.id, false, false);
                    $('#sub_unit_ids').append(newOption).trigger('change');
                    $selected = $('#sub_unit_ids').val();
                    $selected.push(result.data.id);
                    $('#sub_unit_ids').val($selected).trigger('change');
                    if (typeof updateSubUnitPriceTable == 'function') {
                        updateSubUnitPriceTable();
                    }
    ...
    $('.contact_modal').on('shown.bs.modal', function(e) {
        ...
        var cache = {};
        $(".contact_modal").find('#country').autocomplete({
            minLength: 2,
            appendTo : ".contact_modal",
            source: function( request, response ) {
                ...
                $.getJSON( "/get-countries", request, function( data, status, xhr ) {
                    cache[ term ] = data;
                    response( data );
                });
            }
        });
        $(".contact_modal").find('#state').autocomplete({
            minLength: 2,
            appendTo : ".contact_modal",
            source: function( request, response ) {
                var term = request.term;
                $country = $(".contact_modal").find('#country').val();
                $.getJSON( "/get-states?country="+$country, request, function( data, status, xhr ) {
                    response( data );
                })
               ;
            }
            ...
            .validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            $('.contact_modal').trigger('change', result.data);
    ...
    $(document).on('click', '.add_new_customer_btn', function(e) {
        ...
        $('div.contact_modal').load($(this).attr('href'), function() {
            $(this).modal('show');
            $('input:radio[name="contact_type_radio"]').filter('[value="individual"]').attr('checked', true);
            $('.individual').show();
            $('.contact_modal #first_name[name="first_name"]').focus().select();
        });
    });
    ...
    if ($('#header_text2').length) {
        init_tinymce('header_text2');
    }
    ...
    expense_table = $('#expense_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                d.added_by = $('select#added_by').val();
                d.location_id = $('select#location_id').val();
                d.project_id = $('select#project_id').val();
        ...
        columns: [
            { data: 'mass_select',class:"mass_select_column" ,orderable: false,},
            ...
            { data: 'account_name', name: 'ACC.name', orderable: false },
            { data: 'tax', name: 'tr.name' },
            { data: 'final_total', name: 'final_total' },
            { data: 'payment_due', name: 'payment_due' },
            { data: 'amount_due_vv', name: 'amount_due_vv' , orderable: false, searchable: false},
            { data: 'expense_for', name: 'expense_for' },
            { data: 'contact_name', name: 'c.name' },
            { data: 'additional_notes', name: 'additional_notes' },
            { data: 'added_by', name: 'usr.first_name'},
            { data: 'project_name', name: 'PRJ.name'}
            //PRJ.name
        ],
    ...
    $('select#location_id, select#project_id, select#expense_for, select#added_by, select#expense_contact_filter, select#expense_category_id, select#expense_payment_status').on(
        'change',
        function() {
            expense_table.ajax.reload();
        }
    );
    // for closing all the datetimepicker close
    // $(document).on('dp.change', function (e) {
    //     try{
    //         $('#'+(e.target.id)).data('DateTimePicker').hide();
    //     }
    //     catch (e) {
    //     }
    // });
    ...
    //Start: CRUD for income category
    //Income income table
    ...
    income_table = $('#income_table').DataTable({
        ...
        ajax: {
            url: '/incomes',
            data: function(d) {
                d.income_for = $('select#income_for').val();
                d.contact_id = $('select#income_contact_filter').val();
                d.added_by = $('select#added_by').val();
                d.location_id = $('select#location_id').val();
                d.project_id = $('select#project_id').val();
                d.income_category_id = $('select#income_category_id').val();
                d.payment_status = $('select#income_payment_status').val();
        ...
        columns: [
            ...
            { data: 'payment_due', name: 'payment_due' },
            { data: 'amount_due_vv', name: 'amount_due_vv' , orderable: false, searchable: false},
            { data: 'income_for', name: 'income_for' },
            { data: 'contact_name', name: 'c.name' },
            { data: 'additional_notes', name: 'additional_notes' },
            { data: 'added_by', name: 'usr.first_name'},
            { data: 'project_name', name: 'PRJ.name'}
        ...
        fnDrawCallback: function(row, data, start, end, display) {
            ...
            var total_due = sum_table_col($('#income_table'), 'payment_due');

            $('.footer_income_total').html(__currency_trans_from_en(income_total));
            $('.footer_total_due').html(__currency_trans_from_en(total_due));

            $('.footer_payment_status_count').html(
                __sum_status_html($('#income_table'), 'payment-status')
            );
        },
        createdRow: function(row, data, dataIndex) {
            $(row)
    ...
    $('select#location_id, select#project_id, select#income_for, select#added_by, select#income_contact_filter, select#income_category_id, select#income_payment_status').on(
        'change',
        function() {
            income_table.ajax.reload();
        }
    ...
    // });
    //Date picker
    $('#income_transaction_date').datetimepicker({
        format: moment_date_format + ' ' + moment_time_format,
        ignoreReadonly: true,
    });
    ...
    if ($('form#add_invoice_layout_form').length > 0) {
        $('select#design').change(function() {
            if ($(this).val() == 'columnize-taxes' || $(this).val() == 'industrial_1'|| $(this).val() == 'columnize-taxes-new') {
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result) {
                    ...
                    $('.btn-subunit-add').attr('data-href','/units/create?quick_add=0&unit_id='+unit_id);
                }
            },
        });
    }
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.total_unread != 0 ) {
                    ...
                    favicon.badge(data.total_unread);
                }
...
$(document).on('change', 'input#income_final_total, #add_income_modal_form .payment-amount', function() {
...
function calculateIncomePaymentDue() {
    var final_total = __read_number($('input#income_final_total'));
    var payment_amount = __read_number($('#add_income_modal_form input.payment-amount'));
    var payment_due = final_total - payment_amount;
    $('#income_payment_due').text(__currency_trans_from_en(payment_due, true, false));
}
...
$('#pay_components_array').on('select2:select', function (e) {
    var data = e.params.data;
    $html = '<div class="col-lg-6  mb-12 pay_components_array_items" data-id="'+data.id+'">\n' +
        '                            <label>'+data.text+'</label>\n' +
        '                            <input class="form-control" name="comp['+data.id+']" value=""  placeholder="Leave Blank for default amount">\n' +
        '                        </div>';
    $('.pay_components_array_inserts').append($html)
});

$('#pay_components_array').on('select2:unselecting', function (e) {
    var data = e.params.args.data;
    $('.pay_components_array_items[data-id="'+data.id+'"]').remove();
    // console.log(data);
});


function initUserSelectAjax(){
    $('.user_select_ajax').select2({
        ajax: {
            url: '/users-search/find',
            dataType: 'json',
            delay: 250,
            data: function(params) {
                return {
...
function initProductSelectAjax(){
    $('.product_select_ajax').select2({
        minimumInputLength: 2,
        allowClear: true,
        placeholder: '',
        ajax: {
            ...
            processResults: function(data) {
                return {
                    results: $.map(data, function(obj) {
                        ...
                        string += ' (' + obj.sub_sku + ')';
                        return { id: obj.product_id, text: string };
                    })
                };
            },
        },
