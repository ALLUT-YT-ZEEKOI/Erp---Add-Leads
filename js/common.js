...
$(document).ready(function() {
    ...
    if ($('input#__display_precision').length > 0) {
        __display_precision = $('input#__display_precision').val();
    } else {
        __display_precision = 2;
    }
    ...
    jQuery.extend($.fn.dataTable.defaults, {
        ...
        // aLengthMenu: [[10,15,25, 50, 100, 200, 500, 1000, -1], [10,15,25, 50, 100, 200, 500, 1000, LANG.all]],
...
if (localStorage.getItem("startdate") !== null) {
    financial_year.start =  moment(localStorage.getItem("startdate"));
}
if (localStorage.getItem("enddate") !== null) {
    financial_year.end = moment(localStorage.getItem("enddate"));
}
...
//Ask for confirmation for ajax
$(document).on('click', 'a.ajax_link_confirmation', function(e) {
    e.preventDefault();
    swal({
        title: LANG.sure,
        icon: 'warning',
...
//Ask for confirmation for ajax
$(document).on('click', 'a.ajax_confirmation', function(e) {
    e.preventDefault();
    swal({
        title: LANG.sure,
        icon: 'warning',
...
function saveColumnVisibility($variableString , $table_id,  $datatable_obj) {
    $('#'+$table_id).on( 'column-visibility.dt', function ( e, settings, column, state ) {
        $array = [];
        $datatable_obj.columns().visible().each(function (v,k) {
            if(!v){
                $array.push(k)
            }
        });
        localStorage.setItem($variableString, $array);
    } );

    if (localStorage.getItem($variableString) !== null) {
        if (localStorage.getItem($variableString)){
            $datatable_obj.columns(localStorage.getItem($variableString).split(',')).visible(false)
        }
    }
}