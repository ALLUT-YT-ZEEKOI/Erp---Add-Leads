$(document).ready(function() {
    ...
    //contact report
    ageingReport_report_tbl = $('#ageingReport_report_tbl').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            data: function(d) {
                ...
                d.converted_by = $('#converted_by').val();
                d.contact_type = $('#contact_type').val();
                if($('#list_filter_date_range').length){

                    var start = $('input#list_filter_date_range')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD');
                    var end = $('input#list_filter_date_range')
                        .data('daterangepicker')
        ...
        columnDefs: [
            ...
            { targets: [ 2, 3, 4,5,6,7,9], orderable: false,searchable: false },
        ],
        columns: [
            { data: 'name', name: 'name' },
            { data: 'total_invoice', name: 'total_invoice' },
            { data: 'due_today', name: 'due_today' },
            { data: 'due_7_days', name: 'due_7_days' },
            { data: 'due_30_days', name: 'due_30_days' },
            { data: 'due_60_days', name: 'due_60_days' },
            { data: 'due_90_days', name: 'due_90_days' },
            { data: 'due_90_plus', name: 'due_90_plus' },
            { data: 'opening_balance_due', name: 'opening_balance_due' },
            { data: 'due', name: 'due' },
        ],
        fnDrawCallback: function(oSettings) {
            var total_purchase = sum_table_col($('#ageingReport_report_tbl'), 'total_purchase');
            $('#footer_total_purchase').text(total_purchase);

            var total_purchase_return = sum_table_col(
                $('#ageingReport_report_tbl'),
                'total_purchase_return'
            );
            $('#footer_total_purchase_return').text(total_purchase_return);

            var total_sell = sum_table_col($('#ageingReport_report_tbl'), 'total_invoice');
            $('#footer_total_sell').text(total_sell);

            var total_sell_return = sum_table_col($('#ageingReport_report_tbl'), 'total_sell_return');
            ...
            var total_due = sum_table_col($('#ageingReport_report_tbl'), 'total_due');
            $('#footer_total_due').text(total_due);
            $('#footer_total_due_today').text(sum_table_col($('#ageingReport_report_tbl'), 'total_due_today'));
            $('#footer_total_due_7_days').text(sum_table_col($('#ageingReport_report_tbl'), 'total_due_7_days'));
            $('#footer_total_due_30_days').text(sum_table_col($('#ageingReport_report_tbl'), 'total_due_30_days'));
            $('#footer_total_due_60_days').text(sum_table_col($('#ageingReport_report_tbl'), 'total_due_60_days'));
            $('#footer_total_due_90_days').text(sum_table_col($('#ageingReport_report_tbl'), 'total_due_90_days'));
            $('#footer_total_due_90_plus').text(sum_table_col($('#ageingReport_report_tbl'), 'total_due_90_plus'));

            __currency_convert_recursively($('#ageingReport_report_tbl'));
        },
    });
    if($('#ageingReport_report_tbl').length != 0){
        $('#cnt_customer_group_id, #contact_type, #converted_by').change(function() {
            ageingReport_report_tbl.ajax.reload();
        });
    }
    ...
    supplier_report_tbl2 = $('#supplier_report_tbl2').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            var total_purchase = sum_table_col($('#supplier_report_tbl2'), 'total_purchase');
            $('#footer_total_purchase').text(total_purchase);
            ...
            var total_sell = sum_table_col($('#supplier_report_tbl2'), 'total_invoice');
            $('#footer_total_sell').text(total_sell);

            var total_sell_return = sum_table_col($('#supplier_report_tbl2'), 'total_sell_return');
            $('#footer_total_sell_return').text(total_sell_return);
            ...
            var total_due = sum_table_col($('#supplier_report_tbl2'), 'total_due');
            $('#footer_total_due').text(total_due);
    ...
    if($('#supplier_report_tbl2').length != 0){
        $('#cnt_customer_group_id, #contact_type, #list_filter_date_range').change(function() {
        ...
        $('#list_filter_date_range').daterangepicker(dateRangeSettings, function(start, end) {
            supplier_report_tbl2.ajax.reload();
        });
    }
        ...
        sr_sales_commission_report = $('table#sr_sales_with_commission_table').DataTable({
            ...
            ajax: {
                ...
                data: function(d) {
                    ...
                    (d.commission_agent = $('select#sr_id').val()), d.commission_agent_report = true,
                        (d.location_id = $('select#sr_business_id').val()),
                        (d.start_date = start),
                        (d.end_date = end);
            ...
            columns: [
                ...
                { data: 'total_paid', name: 'total_paid' },
                { data: 'total_remaining', name: 'total_remaining' },
                { data: 'sale_commission_agent_name', name: 'commission_agent' },
    ...
    if ($('#product_purchase_report_form #product_pr_date_filter').length == 1) {
        $('#product_purchase_report_form #product_pr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            $('#product_purchase_report_form #product_pr_date_filter').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );
        ...
        $('#product_purchase_report_form #product_pr_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#product_purchase_report_form #product_pr_date_filter').val('');
            product_purchase_report.ajax.reload();
        });
    }
    $(
        '#product_purchase_report_form #variation_id, #product_purchase_report_form #location_id, #product_purchase_report_form #is_asset_other, #product_purchase_report_form #supplier_id, #product_purchase_report_form #product_pr_date_filter'
    ).change(function() {
        product_purchase_report.ajax.reload();
    });
    product_purchase_report = $('table#product_purchase_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                d.is_asset_other = $('select#is_asset_other').val();
            },
        },
        columns: [
            ...
            { data: 'taxtotal', name: 'purchase_lines.item_tax' },
            { data: 'subtotal', name: 'subtotal', searchable: false },
        ],
        fnDrawCallback: function(oSettings) {
            $('#footer_taxtotal').text(
                sum_table_col($('#product_purchase_report_table'), 'row_taxtotal')
            );
            $('#footer_subtotal').text(
                sum_table_col($('#product_purchase_report_table'), 'row_subtotal')
            );
            $('#footer_taxable_value').text(
                sum_table_col($('#product_purchase_report_table'), 'row_taxable_value')
            );
    ...
    if ($('#opening_report_form #product_pr_date_filter').length == 1) {
        $('#opening_report_form #product_pr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            $('#opening_report_form #product_pr_date_filter').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );

            localStorage.setItem("startdate", start);
            localStorage.setItem("enddate", end);
            opening_stock_report_table.ajax.reload();
        });
        $('#opening_report_form #product_pr_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#opening_report_form #product_pr_date_filter').val('');
            opening_stock_report_table.ajax.reload();
        });
    }
    $(
    '#opening_report_form #variation_id, #opening_report_form #location_id, #opening_report_form #supplier_id, #opening_report_form #product_pr_date_filter'
    ).change(function() {
        opening_stock_report_table.ajax.reload();
    });
    opening_stock_report_table = $('table#opening_stock_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[3, 'desc']],
        ajax: {
            url: '/reports/opening-stock-report',
            data: function(d) {
                var start = '';
                var end = '';
                if ($('#product_pr_date_filter').val()) {
        ...
        fnDrawCallback: function(oSettings) {
            $('#footer_subtotal').text(
                sum_table_col($('#opening_stock_report_table'), 'row_subtotal')
            );
            $('#footer_total_purchase').html(
                __sum_stock($('#opening_stock_report_table'), 'purchase_qty')
            );
            $('#footer_total_adjusted').html(
                __sum_stock($('#opening_stock_report_table'), 'quantity_adjusted')
            );
    ...
    //Manufactured Products Report
    if ($('#manufactured_product_report_form #product_pr_date_filter').length == 1) {
        $('#manufactured_product_report_form #product_pr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            $('#manufactured_product_report_form #product_pr_date_filter').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );

            manufactured_product_report_table.ajax.reload();
        });
        $('#manufactured_product_report_form #product_pr_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#manufactured_product_report_form #product_pr_date_filter').val('');
            manufactured_product_report_table.ajax.reload();
        });
    }
    $(
    '#manufactured_product_report_form #variation_id, #manufactured_product_report_form #location_id, #manufactured_product_report_form #supplier_id, #manufactured_product_report_form #product_pr_date_filter'
    ).change(function() {
        manufactured_product_report_table.ajax.reload();
    });
    manufactured_product_report_table = $('table#manufactured_product_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[3, 'desc']],
        ajax: {
            url: '/reports/manufactured-product-report',
            data: function(d) {
                var start = '';
                var end = '';
                if ($('#product_pr_date_filter').val()) {
        ...
        columns: [
            ...
            { data: 'unit_purchase_price', name: 'purchase_lines.purchase_price_inc_tax' },
            { data: 'subtotal', name: 'subtotal', searchable: false },
            { data: 'total_production_cost', name: 'total_production_cost' },
            { data: 'sell_price_inc_tax', name: 'v.sell_price_inc_tax' },
        ],
        fnDrawCallback: function(oSettings) {
            $('#footer_subtotal').text(
                sum_table_col($('#manufactured_product_report_table'), 'row_subtotal')
            );
            $('#footer_total_purchase').html(
                __sum_stock($('#manufactured_product_report_table'), 'purchase_qty')
            );
            $('#footer_total_adjusted').html(
                __sum_stock($('#manufactured_product_report_table'), 'quantity_adjusted')
            );
            __currency_convert_recursively($('#manufactured_product_report_table'));
        },
    });
    ...
    var is_manufacturer_enabled = $('#is_manufacturer_enabled').length > 0 ? true : false;
    product_sell_report_with_purchase_table = $('table#product_sell_report_with_purchase_table').DataTable({
        ...
        columns: [
            ...
            { data: 'manufacturer_name', name: 'manufacturer.name', visible: is_manufacturer_enabled},
