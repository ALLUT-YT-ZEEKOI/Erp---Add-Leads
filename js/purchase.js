$(document).ready(function() {
    $('.expense_inc_tax').trigger('change');
    ...
    if ($('#search_product').length > 0) {
        var disable_stock_check = $('#disable_stock_check').length && $('#disable_stock_check').val() == '1' ? false : true;
        $('#search_product')
            .autocomplete({
                source: function(request, response) {
                    $.getJSON(
                        '/purchases/get_products',
                        {
                            ...
                            term: request.term,
                            check_enable_stock : disable_stock_check
                        },
                        response
                    );
                },
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        columns: [
            ...
            { data: 'new_ref_no', name: 'new_ref_no' },
    ...
    $(document).on(
        ...
        '#purchase_list_filter_location_id, \
                    #purchase_list_filter_supplier_id, #purchase_list_filter_payment_status, #project_id,\
...
function append_purchase_lines(data, row_count) {
    ...
    $('#purchase_entry_table').trigger('change');
}
...
function update_table_total() {
    ...
    $('input#new_total').on('keyup',function () {
        $updatedVal = __read_number($('input#new_total'), true);
        ...
        if(parseFloat(grand_total) != parseFloat($updatedVal) ){
            $('#before_roundoff').show();
        }
        else{
            $('#before_roundoff').hide();
        }

        $('span#before_roundoff_amt').text(__currency_trans_from_en( grand_total , true, true));
        __write_number($('input#grand_total_hidden'), $updatedVal, true);
        __write_number($('input#round_off_amount'), parseFloat($updatedVal)- parseFloat(grand_total) , true);
        $('#grand_total').text(__currency_trans_from_en( $updatedVal , true, true));
    });

function update_grand_total(action='') {
    var st_before_tax = __read_number($('input#st_before_tax_input'), true);
    var total_subtotal = __read_number($('input#total_subtotal_input'), true);
    ...
    if(action !='edit') {
        ...
        __write_number($('input#new_total'), grand_total, true);
        __write_number($('input#round_off_amount'), 0, true);
        $('#grand_total').text(__currency_trans_from_en(grand_total, true, true));
    }
    // grand_total = parseFloat($('input#grand_total_hidden').val());
...
$('.expense_without_tax').on('keyup change', function () {
    $taxRate = parseFloat($(this).closest('tr').find('.tax_select :selected').attr('data-rate'));
    $val = parseFloat($(this).val());
    if($taxRate > 0){
       $tax = ($val * $taxRate)/100;
        $(this).closest('tr').find('.expense_tax').val($tax.toFixed(2));
        $(this).closest('tr').find('.expense_inc_tax').val(($val+$tax).toFixed(2));
    }
    else{
        $(this).closest('tr').find('.expense_tax').val(0);
        $(this).closest('tr').find('.expense_inc_tax').val($val);
    }
});

$('.expense_without_tax').on('change', function () {
    $('.expense_inc_tax').trigger('change');
});

$('.tax_select').on('change', function () {
    $('.expense_without_tax').trigger('keyup');
    $('.expense_inc_tax').trigger('change');
});

$('.expense_inc_tax').on('keyup change', function () {
    $taxRate = parseFloat($(this).closest('tr').find('.tax_select :selected').attr('data-rate'));
    $val = parseFloat($(this).val());
    if($taxRate > 0) {
        $unitPrice = __get_principle($val, $taxRate) ;
        $(this).closest('tr').find('.expense_tax').val(($val-$unitPrice).toFixed(2));
        $(this).closest('tr').find('.expense_without_tax').val(($unitPrice).toFixed(2));
    }
    else{

        $(this).closest('tr').find('.expense_without_tax').val($val);
    }
});
...
$('#shipping_charges_before_tax').on('keyup change', function () {
...
$('#shipping_charges').on('keyup change', function () {
