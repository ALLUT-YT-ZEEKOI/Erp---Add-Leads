...
$(document).on('click','.gradientText', function () {
   $(this).closest('.product_box').find('.itemselected').attr('style', '');
});
setTimeout(function () {
    $('.loading11').hide();
},3500);
...
function changeCustomerName(){
    $name  = $('#customer_id').select2('data')[0].text.trim();
    $id  = $('#customer_id').val();
    $('.current-customer').html($name.substr(0,8) || '');
    if($id > 0) {
        $('.pos_customer_edit').attr('href', '/contacts/' + $id + '/edit');
        $('.pos_customer_edit').show();
    }
    else{
        if($('#default_customer_id').val()>0){
            ('.pos_customer_edit').attr('href', '/contacts/' + $('#default_customer_id').val() + '/edit');
            $('.pos_customer_edit').show();
        }
        else{
            $('.pos_customer_edit').hide();
        }
    }
...
$(document).ready(function() {
    ...
    $('.loading11').fadeOut(500);
// $('#product_category').select2({
//     closeOnSelect: false
// });

    $('.product_category_class option').each(function(){
        localStorage.removeItem('category_id'+$(this).attr('value')+'page1');
    });
    ...
    $('#product_category').select2('open');

    $('#select2-product_category-results').closest('.select2-container').addClass('customSelect2');
    $('#select2-product_category-results').closest('.select2-dropdown').addClass('customSelect2');


    $( "#searchCustomerModal").on('shown.bs.modal', function(){
        $('#customer_id').select2('open');
    });
    $( "#restaurantTableModal").on('shown.bs.modal', function(){
        $val = $(document).find('[name="res_table_id"]').val();
        $('.tablebutton').removeClass('active');
        $('.tablebutton[data-id="'+$val+'"]').addClass('active');
    });

    $('.tablebutton').on('click', function () {
        $('.tablebutton').removeClass('active');
        $(this).addClass('active');
        $(document).find('[name="res_table_id"]').val($(this).data('id')).trigger('change');
        $('.current-table').html($(this).html());
        $('#restaurantTableModal').modal('hide');
    });


    $( "#serviceStaffModal").on('shown.bs.modal', function(){
        $val = $(document).find('[name="res_waiter_id"]').val();
        $('.waiterbutton').removeClass('active');
        $('.waiterbutton[data-id="'+$val+'"]').addClass('active');
    });

    $('.waiterbutton').on('click', function () {
        $('.waiterbutton').removeClass('active');
        $(this).addClass('active');
        $(document).find('[name="res_waiter_id"]').val($(this).data('id')).trigger('change');
        $('#serviceStaffModal').modal('hide');
        $name  = $(this).html().trim();
        $('.current-staff').html($name.substr(0,8) || '');
    });

    $( "#commissionAgentModal").on('shown.bs.modal', function(){
        $val = $(document).find('[name="commission_agent"]').val();
        $('.commissionAgentButton').removeClass('active');
        $('.commissionAgentButton[data-id="'+$val+'"]').addClass('active');
    });

    $('.commissionAgentButton').on('click', function () {
        $('.commissionAgentButton').removeClass('active');
        $(this).addClass('active');
        $(document).find('[name="commission_agent"]').val($(this).data('id')).trigger('change');
        $('#commissionAgentModal').modal('hide');
        $name  = $(this).html().trim();
        $('.current-commssionAgent').html($name.substr(0,8) || '');
    });


    $( "#priceGroupModal").on('shown.bs.modal', function(){
        $val = $(document).find('[name="price_group"]').val();
        $('.pricegroupbutton').removeClass('active');
        $('.pricegroupbutton[data-id="'+$val+'"]').addClass('active');
    });

    $('.pricegroupbutton').on('click', function () {
        $('.pricegroupbutton').removeClass('active');
        $(this).addClass('active');
        $(document).find('[name="price_group"]').val($(this).data('id')).trigger('change');
        $('#priceGroupModal').modal('hide');
        $name  = $(this).html().trim();
        $('.current-priceGroup').html($name.substr(0,8) || '');
    });

    $( "#serviceTypeModal").on('shown.bs.modal', function(){
        $val = $(document).find('[name="types_of_service_id"]').val();
        $('.servicebutton').removeClass('active');
        $('.servicebutton[data-id="'+$val+'"]').addClass('active');
    });

    $('.servicebutton').on('click', function () {
        $('.servicebutton').removeClass('active');
        $(this).addClass('active');
        $(document).find('[name="types_of_service_id"]').val($(this).data('id')).trigger('change');
        $('#serviceTypeModal').modal('hide');
        $name  = $(this).html().trim();
        $('.current-serviceType').html($name.substr(0,8) || '');
    });
                ...
                $.ajax({
                    ...
                    success: function(result){
                        if(result.success == true){
                            ...
                            $this.html('<i class="fa fa-check"></i>  Served ');
    ...
    $( "#orderCustomModal").on('shown.bs.modal', function(){
        console.log('order_modal');
        $(this).find('.modal-body > .row').html('<div class="text-center width-100"> Loading.. Please Wait..</div>');
        $.ajax({
            context : $(this),
            method: 'GET',
            url: '/sells/pos/get-orders-data',
            dataType: 'html',
            success: function(result) {
                $(this).find('.modal-body > .row').html(result);
                $('.order_item').hide();
                $('.order-filter.active').each(function () {
                    $(this).closest('.modal').find('.order_item.'+$(this).data('filter')).slideToggle();
                });
                if($('.order-filter.active').length == 0){
                    $('.order-filter[data-filter="due"]').trigger('click');
                }
            },
        });
    });
    ...
    $(document).on('click','.order-filter', function () {
        ...
        $(this).closest('.modal').find('.order_item.'+filter).slideToggle();
    });
        ...
        language: {
            noResults: function() {
                ...
                return (
                    ...
                    name +
                    '" class="btn btn-link add_new_customer add_new_customer_btn" href="/contacts/create?num='+name+'"><i class="fa fa-plus-circle fa-lg" aria-hidden="true"></i>&nbsp; ' +
                ...
                response: function(event, ui) {
                    if (ui.content.length == 1) {
                       ...
                       // toastr.error(LANG.no_products_found);
                        ...
                        pos_product_row(null, null, $('#search_product').val());
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        adjustComboQty(tr);
        tr.find('.row_discount_amount').trigger('change');
    });
    ...
    $(document).on('change','.discount-selector', function () {

        $(this).closest('tr').find('.discount_id').val($(this).find(':selected').data('id'));
        $(this).closest('.modal').find('.row_discount_type').val($(this).find(':selected').data('type'));
        $(this).closest('.modal').find('.row_discount_amount').val($(this).find(':selected').data('amt'));
        $(this).closest('.modal').find('.row_discount_type').trigger('change');
        $(this).closest('.modal').find('.row_discount_amount').trigger('change');
        $(this).closest('.modal').find('.discountOfferText').html("Offer Applied : "+ $(this).find(':selected').text());

    });

    $(document).on('click','.lot-selector', function () {
        $('.product_row[data-row_index="'+$(this).data('rowcount')+'"]').find('.lot_number').val($(this).data('id')).trigger('change');
        // $('#multiprice_modal_'+$(this).data('rowcount')).modal('toggle');
        $(this).closest('.multiprice_modal').modal('hide');
        // $(document).find('.multiprice_modal').modal('hide');
    });
    ...
    $('table#pos_table tbody').on('change', 'select.supplier_lot_id', function() {
        var supp_id =  $(this).val();
        var lot =$(this).closest('tr').find('.lot_number').find('option[data-supplier_id='+supp_id+']').val();
        $(this).closest('tr').find('.lot_number').val(lot).trigger('change');
    });
    ...
    $('table#pos_table tbody').on('change', 'select.lot_number', function() {
        ...
        var supp_id =  $(this).find(':selected').data('supplier_id');
        if(supp_id){
            $(this).closest('tr').find('.supplier_lot_id').val(supp_id);
        }
        else{
            $(this).closest('tr').find('.supplier_lot_id').val('');
        }
        ...
        if (sub_unit_length > 0) {
            ...
            tr.find('select.sub_unit').val(tr.find('.product_base_unit_id').val());
        }
        ...
        if ($(this).val() && !allow_overselling) {
            ...
            var new_sell_price = $('option:selected', $(this)).data('sell-price');
            __write_number(tr.find('input.pos_unit_price_inc_tax'), new_sell_price);

            tr.find('input.pos_unit_price_inc_tax').trigger('change');
            tr.find('input.hidden_base_unit_sell_price').val(tr.find('input.pos_unit_price ').val())

            $discountAmtTemp  = tr.find('.row_discount_amount').val();
            tr.find('.row_discount_amount').val(0);
            tr.find('input.pos_unit_price_inc_tax').trigger('change');
            tr.find('.row_discount_amount').val($discountAmtTemp);
            // tr.find('.row_discount_amount').trigger('change');
            if (sub_unit_length > 0) {
        ...
        tr.find('select.sub_unit').trigger('change');
    ...
    $('button.pos-express-finalize').click(function() {
        // Check Table
        if($(document).find('[name="res_table_id"]').val() == undefined){
            //table not enabled
        }

        if($(document).find('[name="res_table_id"]').val() == "" && $('.isTablePopUpEnabled').val() == 1){
            $( "#restaurantTableModal").modal('show');
            toastr.error("Please select Table");
            return false;
        }

        if($(document).find('[name="res_waiter_id"]').val() == "" && $('.isServiceStaffTopUpEnabled').val() == 1){
            $( "#serviceStaffModal").modal('show');
        ...
        $(document).find('.kotprintenable').remove();
        //If pay method is credit sale submit form
        if (pay_method == 'credit_sale') {
            if($(this).hasClass('kotprint')){
                pos_form_obj.append('<input class="kotprintenable" name="kotprint" value="1" type="hidden">');
            }
            else{
                $(document).find('.kotprintenable').remove();
            }
    ...
    pos_form_validator = pos_form_obj.validate({
        submitHandler: function(form) {
            ...
            if (cnf) {
                ...
                if($(form).find('[name="type"]').val() == 'sales_order'){
                    data = data + '&status=ordered';
                }
                else{
    ...
    $('button#posEditDiscountModalUpdate').click(function() {
        ...
        //Apply Discount from Bottom
        $('table#pos_table tbody tr').each(function() {
            this_row = $(this);
            this_row.find('.row_discount_type').val('percentage');
            this_row.find('.row_discount_amount ').val($('input.discount_amount_modal_new_discount').val());
            this_row.find('.row_discount_amount ').trigger('change');
        });
        pos_total_row();
    });
    ...
    $('.contact_modal').on('hidden.bs.modal', function() {
        ...
        if (typeof $('form#quick_add_contact')[0] !== 'undefined') {
    ...
    $('#datetimepicker12').datetimepicker({
        ...
        var formatted_date = event.date.format(moment_date_format + ' ' + moment_time_format);
        $('#transaction_date').val(formatted_date);
        $('#transactiondateModal').modal('hide');
        $name = event.date.format('D MMM YY');
        $('.current-date').html($name || '');
    });
    ...
    $('[name="term"]').on('keydown', function (e) {
        if(e.keyCode == 13){
    ...
    $('#search_product').keydown(function(e) {
        ...
        if(key ==13){
            ...
            $('#search_product').val('');
            if($term){
                pos_product_row(null, null, $term);
            }
        }
    });
    ...
    $('table#pos_table').on('change', 'select.sub_unit', function() {
        ...
        adjustComboQty(tr);
        var subunitprice = parseFloat(selected_option.data('subunitprice'));
        if(subunitprice){
            tr.find('input.pos_unit_price_inc_tax').val(subunitprice);
            tr.find('input.pos_unit_price_inc_tax').trigger('change');
        }
    });
    ...
    $(window).resize(function() {
        ...
        div_height = __calculate_amount('percentage', 60, win_height);
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null, term=null) {
    ...
    if(term == ''  || term == null){
    //     if (page == 1) {
            dataTest = localStorage.getItem('category_id'+category_id+'page'+page+'location_id'+location_id);
            if(dataTest){
                if (page == 1) {
                    $('div#product_list_body').html('');
                }
                $('div#product_list_body').append(dataTest);
                $('#suggestion_page_loader').fadeOut(700);
                markSelectedItems();
                hasCache = true;
            }
    ...
    if(!hasCache){
        ...
        currentXhr = $.ajax({
            ...
            success: function(result) {
                ...
                if(term == '' || term == null) {
                    localStorage.setItem('category_id' + category_id + 'page' + page + 'location_id' + location_id, result);
                }
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    ...
    if (variation_id == null && weighing_scale_barcode != null) {
        if ($('#pos_table tbody').find('tr[data-sku="' + weighing_scale_barcode + '"]').length > 0) {
            $variation_id_weign = $('#pos_table tbody').find('tr[data-sku="' + weighing_scale_barcode + '"]').find('.row_variation_id').val();
            if ($variation_id_weign > 0) {
                variation_id = $variation_id_weign;
            }
        }
    }
    ...
    if (item_addtn_method == 0) {
        ...
        $('#pos_table tbody')
            ...
            .each(function() {
                ...
                var row_weighing_scale_barcode = $(this)
                    .data('sku');

                if(weighing_scale_barcode != null){
                    if ( row_weighing_scale_barcode == weighing_scale_barcode &&
                        enable_sr_no !== '1' &&
                        !modifiers_exist &&
                        !is_added
                    ) {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (result.success) {
                    ...
                    //Apply Discount from Bottom
                    if($('input.discount_amount_modal_new_discount').val() > 0){
                        if($('input.discount_amount_modal_new_discount').val() > this_row.find('.row_discount_amount').val()){
                            this_row.find('.row_discount_type').val('percentage');
                            this_row.find('.row_discount_amount ').val($('input.discount_amount_modal_new_discount').val());
                        }
                    }
                    this_row.find('.row_discount_type').val('percentage');
                    this_row.find('.row_discount_amount ').val();
...
function markSelectedItems() {
    ...
    $('.product_row').each(function () {
        variation_id  = $(this).find('.row_variation_id').val();
        $productQtyArray[variation_id] = parseFloat($productQtyArray[variation_id] || 0) + parseFloat($(this).find('.input_quantity').val());
    });

    $('.itemselected').hide();
    for(k in $productQtyArray){
        $('[data-variation_id="'+k+'"]').find('.itemselected').show();
        $('[data-variation_id="'+k+'"]').find('.itemselectedCount').html($productQtyArray[k]);
    };
}
...
function pos_each_row(row_obj) {
    ...
    row_obj.find('.discount-show-text').html('');
    if (discount > 0) {
        ...
        var actual_unit_price = __read_number(row_obj.find('input.pos_unit_price'));

        $totaldiscount_price = actual_unit_price - discounted_unit_price;
        if($totaldiscount_price > 0){
            $totaldiscount_price = $totaldiscount_price * qty;
            row_obj.find('.discount-show-text').html('<small>Discount.</small> : <span data-org="'+$totaldiscount_price+'">'+ __currency_trans_from_en($totaldiscount_price, true)+'</span>');
        }
...
function calculateTotalDiscount(){
    ...
    $('table#pos_table tbody tr').each(function() {
        this_row = $(this);
        $totalDiscount += this_row.find('.discount-show-text span').data('org');
    });
    $('#total_discount2').html(__currency_trans_from_en($totalDiscount, true));
}
...
function pos_total_row() {
    ...
    $('span.price_total').html(__currency_trans_from_en(price_total, false, false, __display_precision));
...
function calculate_billing_details(price_total) {
    ...
    $('span#total_payable').text(__currency_trans_from_en(shown_total, false ,false, __display_precision));
    $('span.total_payable_span').text(__currency_trans_from_en(total_payable_rounded, true,false, __display_precision));
...
function reset_pos_form(){
    ...
    $('#billing_address_div').html('');
    $('#shipping_address_div').html('');
    $('#commission_agent').trigger('change');
    $('[name="converted_by"]').trigger('change');
...
function set_default_customer(fullreset = false) {
    ...
    if($('select#customer_id').val() && !fullreset){
        $('select#customer_id')
            .trigger('change');
    }
    else{
...
function changeLocation($id){
    $.ajax({
        method: 'GET',
        url: '/business-location/get-settings/'+$id,
        dataType: 'json',
        success: function(result) {

            $('.servicebutton').hide();
            $types = (result.types_of_service);
            ...
            if($types.length == 0){
                ...
                $.each($types, function (k,v) {
                   $('.servicebutton[data-id="'+v+'"]').show();
                   if($types.length == 1){
                       $('.servicebutton[data-id="'+v+'"]').trigger('click');
                   }
                });
            }
            if(result.shipping_status){
                $('#shipping_status').val(result.shipping_status);
            }
            if(result.shipping_status){
                $('#invoice_layout_id').val(result.invoice_layout_id);
                $('#invoice_layout_id').trigger('change');
            }
        },
    });
...
$('#transport_cost_bearing, #shipping_charges_vv').on('keyup', function () {
    $shippingCharge = parseFloat($('#shipping_charges_vv').val().replace(',', '')) || 0;
    $transport_cost_bearing = parseFloat($('#transport_cost_bearing').val().replace(',', '')) || 0;
    $bearingbyCustomer = $shippingCharge * ($transport_cost_bearing/100);
    $bearingbyCompany = $shippingCharge - $bearingbyCustomer;
    if($bearingbyCompany > 0){
        $('.transport_cost_bearing_comment').html((100 -$transport_cost_bearing) + "% of cost will be added to company expense. ie."+ $bearingbyCompany )
    }
    else{
        $('.transport_cost_bearing_comment').html('');
    }


    $('#shipping_charges').val($bearingbyCustomer);
    $('#shipping_charges').trigger('change');
    changeBeforeTax($('#shipping_charges'));

});

$('#shipping_charges').on('keyup', function () {

    changeShippingChangeTotal();
});

function changeShippingChangeTotal(){
    $tcbText = $('#transport_cost_bearing').val();
    if($tcbText){
        $transport_cost_bearing = parseFloat($tcbText.replace(',', ''));
        $val = parseFloat($('#shipping_charges').val()|| 0);
        $totalVal = ($val * 100) / $transport_cost_bearing;
        $('#shipping_charges_vv').val($totalVal);
    }
}


$(document).on('keyup change','#packing_charge_before_tax', function () {

    $taxRate = parseFloat($('.packing_charge_tax_id :selected').attr('data-rate'));
    $val = parseFloat($(this).val());
    if($taxRate > 0){
        $tax = ($val * $taxRate)/100;
        $('.packing_charge_tax').val($tax.toFixed(2));
        $('#packing_charge').val(($val+$tax).toFixed(2));
    }
    else{
        $('.packing_charge_tax').val(0);
        $('#packing_charge').val($val);
    }
});


$(document).on('change','#packing_charge_before_tax', function () {
    $('#packing_charge').trigger('change');
});

$(document).on('change', '.packing_charge_tax_id', function () {
    $('#packing_charge_before_tax').trigger('keyup');
    // $('#shipping_charges').trigger('change');
});

$(document).on('keyup change', '#packing_charge', function () {

    $taxRate = parseFloat($('.packing_charge_tax_id :selected').attr('data-rate'));
    $val = parseFloat($(this).val());
    if($taxRate > 0) {
        $unitPrice = __get_principle($val, $taxRate) ;
        $('.packing_charge_tax').val(($val-$unitPrice).toFixed(2));
        $('#packing_charge_before_tax').val(($unitPrice).toFixed(2));
    }
    else{

        $('#packing_charge_before_tax').val($val);
    }
});



$('#shipping_charges_before_tax').on('keyup', function () {

    $taxRate = parseFloat($('.shipping_charges_tax_id :selected').attr('data-rate'));
    $val = parseFloat($(this).val());
    if($taxRate > 0){
        $tax = ($val * $taxRate)/100;
        $('.shipping_charges_tax').val($tax.toFixed(2));
        $('#shipping_charges').val(($val+$tax).toFixed(2));
    }
    else{
        $('.shipping_charges_tax').val(0);
        $('#shipping_charges').val($val);
    }
    changeShippingChangeTotal();
    $('#shipping_charges').trigger('change');
});

$('#shipping_charges_before_tax').on('change', function () {
    $('#shipping_charges').trigger('change');
});
$('#shipping_charges').on('change', function () {
    // $('#shipping_charges').trigger('keyup');
});

$('.shipping_charges_tax_id').on('change', function () {
    $('#shipping_charges_before_tax').trigger('keyup');
    // $('#shipping_charges').trigger('change');
});
...
function changeBeforeTax() {

    $taxRate = parseFloat($('.shipping_charges_tax_id :selected').attr('data-rate'));
    $val = parseFloat($('#shipping_charges').val());
    if($taxRate > 0) {
        $unitPrice = __get_principle($val, $taxRate) ;
        $('.shipping_charges_tax').val(($val-$unitPrice).toFixed(2));
        $('#shipping_charges_before_tax').val(($unitPrice).toFixed(2));
    }
    else{

        $('#shipping_charges_before_tax').val($val);
    }
}

$('#invoice_layout_id_pp').on('change', function () {
    $('#invoice_layout_id').val($(this).val()).trigger('change');
