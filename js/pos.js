$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if ($('#types_of_service_id').length > 0){
                    ...
                    $('#types_of_service_id > option').each(function () {
                        ...
                        if($types.length == 0){
                            ...
                            if($types.indexOf(parseInt($(this).val())) != -1)
                            {
                                $(this).show();
                            }
                            else{
    ...
    $(document).on('click','.lot-selector', function () {
        ...
        // $('#multiprice_modal_'+$(this).data('rowcount')).modal('hide');
    ...
    $('table#pos_table tbody').on('change', 'select.lot_number', function() {
        ...
        if ($(this).val() && !allow_overselling) {
            ...
            $('.row_discount_amount').trigger('change');
            tr.find('input.pos_unit_price_inc_tax').trigger('change');
    ...
    $('#sales_type_id').on('change', function(){
        ...
        if(data[0].text == 'PREPAID'){
            $('.form-group.transaction_reference').fadeIn();
            $('.form-group.transaction_reference').find('input').attr('required',"required");
        }
        else{
            $('.form-group.transaction_reference').fadeOut();
            $('.form-group.transaction_reference').find('input').removeAttr('required');
        }
    });

    $('#sales_type_id').trigger('change');
    ...
    $('button#save-and-print-modal').click(function(e) {
        if($('#invoice_layout_id').val() > 0){
            if($('#modal_invoice_'+$('#invoice_layout_id').val()).length > 0){
                $('#modal_invoice_'+$('#invoice_layout_id').val()).modal('show');
            }
            else{
...
$(document).on('change', 'select#customer_id', function(){
    ...
    if ($(this).val() != default_customer_id) {
        ...
        get_sales_orders();
        if (typeof check_sales_orders === 'function') {
            check_sales_orders();
        }
    }
});
...
$(document).on('change', '.payment_types_dropdown', function(e) {
    ...
    if (payment_type == 'credit') {
        $('#amount_'+row_index).val('0');
    }
    else {
        $('#amount_'+row_index).val(__read_number($('#final_total_input')));
    }
...
$( ".contact_modal").on('change', function(e ,data){
    update_shipping_address(data);
});
...
function update_shipping_address(data) {
    ...
    if ($('#shipping_address_div').length) {
        ...
        if (data.mobile) {
            shipping_address += ',<br> Phone :' + data.mobile;
        }

        $btn = '<a href="/contacts/'+data.id+'/edit" class="edit_contact_button btn btn-default mt-5"><i class="glyphicon glyphicon-edit"></i>Edit</a>';
        $('#shipping_address_div').html(shipping_address +'<br>'+ $btn);
    }
    if ($('#billing_address_div').length) {
        ...
        if (data.zip_code) {
            address.push('<br>Pincode: ' + data.zip_code);
        }
        if (data.mobile) {
            address.push('<br>Phone : ' + data.mobile);
        }
        if (data.tax_number) {
            address.push('<br>Tax No : ' + data.tax_number);
        }
    ...
    $('#shipping_address').val(data.name+'\r\n'+data.shipping_address+'\r\n'+'Mobile : '+data.mobile);
    console.log(data);
    ...
    if ($('#sales_order_ids').length) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if($("#so_id").val() > 0){
                    $('#sales_order_ids').select2('val', [$("#so_id").val()]);
...
function set_so_values(so) {
    ...
    $('[name="converted_by"]').val(so.converted_by).trigger('change');
    $('#types_of_service_id').val(so.types_of_service_id);
    if ($('#types_of_service_id').length && $('#types_of_service_id').val()) {
        $('#types_of_service_id').change();
    }
    if ($('#shipping_custom_field_1').length) {
    ...
    if ($('#custom_field_1').length) {
        $('#custom_field_1').val(so.custom_field_1);
    }
    if ($('#custom_field_2').length) {
        $('#custom_field_2').val(so.custom_field_2);
    }
    if ($('#custom_field_3').length) {
        $('#custom_field_3').val(so.custom_field_3);
    }
    if ($('#custom_field_4').length) {
        $('#custom_field_4').val(so.custom_field_4);
    }
}
